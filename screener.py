...
# -----------------------------
# SMART UNIVERSE (LIMITED FOR SPEED)
# -----------------------------
def get_universe():

    url = "https://finnhub.io/api/v1/stock/symbol?exchange=US&token=" + os.environ.get("FINNHUB_API_KEY")
    r = requests.get(url, timeout=20)

    data = r.json()

    # keep only first 100 symbols for performance
    return [x["symbol"] for x in data[:100]]
...
def run_screener():
    ...
    # -----------------------------
    pretty_text = "\n\n".join([
        f"{r['symbol']} | ${r['price']} | {r['change_pct']}% | {r['signal']} | Score: {r['score']}\n→ {r['explanation']}"
        for r in top
    ])
